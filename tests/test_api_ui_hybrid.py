...
def expected_post():
    ...
    response = requests.get(f"{API_BASE}/posts/1", timeout=TIMEOUT)
...
def expected_user():
    ...
    response = requests.get(f"{API_BASE}/users/1", timeout=TIMEOUT)
    assert response.status_code == 200
    return response.json()
...
def new_user():
    ...
    # teardown: in a real project — requests.delete(f"{API_BASE}/users/<id>")
